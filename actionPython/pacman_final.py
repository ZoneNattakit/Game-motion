...
Logger = LoggingFunction('pacman_fin')
...
class GameRenderer:
    ...
    def display_text(self, text, in_position=(20, 0), in_size=30):
    ...
    def restart_game(self):
        ...
        # Add walls, cookies, powerups, and ghosts again
        for y, row in enumerate(pacman_game.numpy_maze):
            for x, column in enumerate(row):
                if column == 0:
                    self.add_wall(Wall(self, x, y, 20))

        for cookie_space in pacman_game.cookie_spaces:
            translated = translate_maze_to_screen(cookie_space)
            cookie = Cookie(self, translated[0] + 10, translated[1] + 10)
            self.add_cookie(cookie)

        for powerup_space in pacman_game.powerup_spaces:
            translated = translate_maze_to_screen(powerup_space)
            powerup = Powerup(self, translated[0] + 10, translated[1] + 10)
            self.add_powerup(powerup)

        for i, ghost_spawn in enumerate(pacman_game.ghost_spawns):
            translated = translate_maze_to_screen(ghost_spawn)
            ghost = Ghost(self, translated[0], translated[1], 20, pacman_game,
                          pacman_game.ghost_colors[i % 4])
            self.add_ghost(ghost)

        pacman = Hero(self, 20, 20, 20)
...
if __name__ == "__main__":
    try:
        ...
        Logger.logInfoMessage("end process")
